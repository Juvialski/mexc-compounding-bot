...
app.get('/', (req, res) => res.send('Elite Hybrid Sniper V3: Scalp & Swing Active'));
app.listen(port);
...
let isTrading = false;
let peakPrice = 0; 
let activeStrategy = 'NONE'; // Tracks if current position is 'SCALP' or 'SWING'

async function getMarketContext() {
    // Fetching 3 timeframes: 1h for macro trend, 15m for swings, 1m for scalps
    ...
    const bb1m = BollingerBands.calculate({ period: 20, values: closes1m, stdDev: 2.5 }).pop(); // Wider deviation for 1m extremes
...
async function runBot() {
    ...
    try {
        ...
        // Fallback: If server restarts while in a trade, assume it is a SWING to be safe
        ...
        // Dynamic Stop Parameters based on active strategy
        const activeAtr = activeStrategy === 'SCALP' ? ctx.scalp.atr : ctx.swing.atr;
        const trailMultiplier = activeStrategy === 'SCALP' ? 1.0 : 1.5; // Tighter trail for scalps
        const stopMultiplier = activeStrategy === 'SCALP' ? 1.5 : 2.0;
        ...
        if (contractsToTrade >= 1 && !longPos && !shortPos && usdtBalance > 5) {
            ...
            // 1. SCALP LOGIC (Highest Priority for fast execution)
            ...
            // 2. SWING LOGIC (Macro Reversals and Safe Trends)
            else {
...
async function startBot() {
    try {
        ...
        console.log(`SUCCESS: ELITE HYBRID SNIPER V3 ACTIVE ON ${symbol}`);
